...
export default function ChatPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
      <main className="patient-chat-app" data-text-size={textSize} style={{ height: "100%", minHeight: "-webkit-fill-available", display: "flex", flexDirection: "column", background: appBg, color: textPrimary, fontFamily: chatFontFamily, overflow: "hidden", maxWidth: "100vw" }}>
        ...
        .staff-exit-link { position: absolute; right: max(12px, env(safe-area-inset-right)); top: 50%; transform: translateY(-50%); width: 50px; height: 50px; min-height: 50px; display: flex; align-items: center; justify-content: center; padding: 7px; border-radius: 999px; border: 1px solid rgba(255,255,255,0.20); background: rgba(255,255,255,0.12); color: #fff; text-decoration: none; font-family: inherit; }
        .staff-exit-link img { width: 34px; height: 34px; object-fit: contain; display: block; }
        @media (max-width: 520px) { .staff-exit-link { right: max(8px, env(safe-area-inset-right)); width: 46px; height: 46px; min-height: 46px; padding: 6px; } .staff-exit-link img { width: 32px; height: 32px; } }
      ...
      <header style={{ position: "relative", height: 88, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: "#0B3C5D", borderBottom: "1px solid rgba(229,231,235,0.65)", padding: "5px 8px", overflow: "hidden" }}>
        <Image src="/fonseca_blue.png" alt="Dr. Fonseca" width={430} height={78} priority style={{ width: "95%", maxWidth: 520, height: "auto", maxHeight: 78, objectFit: "contain", objectPosition: "center" }} />
        {viewerType === "staff" && (
          <a className="staff-exit-link" href="/inbox" title="Salir" aria-label="Salir">
            <Image src="/Exit_icon.png" alt="" width={34} height={34} />
          </a>
        )}
        {viewerType === "patient" && (
          <button
