...
const loadQuickReplies = (roomId: string): QuickReply[] => {
  if (typeof window === "undefined") return [];
  const saved = localStorage.getItem("patient-qr-" + roomId);
  if (!saved) return [];

  try {
    return JSON.parse(saved) as QuickReply[];
  } catch {
    return [];
  }
};
...
export default function PatientPage({ params }: { params: { roomId: string } }) {
  ...
  const [quickReplies, setQuickReplies] = useState<QuickReply[]>(() => loadQuickReplies(roomId));
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100dvh", background: bg, fontSize, color: textColor, fontFamily: "'Segoe UI', sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      {showSlash && filteredQR.length > 0 && (
        <div style={{ position: "absolute", bottom: 68, left: 0, right: 0, background: panelBg, borderTop: `1px solid ${menuBorder}`, zIndex: 50, maxHeight: 200, overflowY: "auto" }}>
          {filteredQR.map((qr, i) => (
            <div key={i} onClick={() => { setText(qr.message); setShowSlash(false); }}
              style={{ padding: "10px 16px", borderBottom: `1px solid ${menuBorder}`, cursor: "pointer" }}>
      ...
      {menuOpen && (
        <div style={{ position: "absolute", bottom: 72, left: 8, background: menuBg, borderRadius: 16, boxShadow: "0 4px 20px rgba(0,0,0,0.18)", border: `1px solid ${menuBorder}`, zIndex: 100, width: 220, overflow: "hidden" }}>
          <button onClick={() => openPicker("image/*;capture=camera")}
            style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", padding: "13px 18px", background: "none", border: "none", cursor: "pointer", fontSize: 15, color: textColor, borderBottom: `1px solid ${menuBorder}` }}>
            <span style={{ fontSize: 20 }}>📷</span>
            Cámara
          </button>
          <button onClick={() => openPicker("image/*,video/*")}
            style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", padding: "13px 18px", background: "none", border: "none", cursor: "pointer", fontSize: 15, color: textColor, borderBottom: `1px solid ${menuBorder}` }}>
          ...
          <button onClick={() => { setShowQR(true); setMenuOpen(false); }}
            style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", padding: "13px 18px", background: "none", border: "none", cursor: "pointer", fontSize: 15, color: textColor, borderBottom: `1px solid ${menuBorder}` }}>
            <span style={{ fontSize: 20 }}>⚡</span>
            Respuestas rápidas
          </button>
          <button onClick={() => { setShowSettings(true); setMenuOpen(false); }}
            style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", padding: "13px 18px", background: "none", border: "none", cursor: "pointer", fontSize: 15, color: textColor }}>
            <span style={{ fontSize: 20 }}>⚙️</span>
            Configuración
          </button>
        </div>
          ...
          <div style={{ background: panelBg, width: "100%", borderRadius: "20px 20px 0 0", padding: "24px 20px 40px", maxHeight: "80dvh", overflowY: "auto" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              ...
              <div onClick={() => setDarkMode(d => !d)}
                style={{ width: 50, height: 28, borderRadius: 14, background: darkMode ? "#075E54" : "#ccc", cursor: "pointer", position: "relative", transition: "background 0.2s" }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <input type="range" min={13} max={20} value={fontSize} onChange={e => setFontSize(Number(e.target.value))}
                style={{ width: "100%", accentColor: "#075E54" }} />
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 200, display: "flex", alignItems: "flex-end" }}>
          <div style={{ background: panelBg, width: "100%", borderRadius: "20px 20px 0 0", padding: "24px 20px 40px", maxHeight: "85dvh", overflowY: "auto" }}>
            ...
            <div style={{ background: darkMode ? "#2C2C2E" : "#f5f5f5", borderRadius: 12, padding: 16, marginBottom: 20 }}>
              ...
              <textarea value={qrMessage} onChange={e => setQrMessage(e.target.value)} placeholder="Mensaje completo..." rows={3}
                style={{ width: "100%", background: inputBg, border: `1px solid ${menuBorder}`, borderRadius: 8, padding: "8px 12px", fontSize: 14, color: textColor, resize: "none", boxSizing: "border-box" as const }} />
              <button onClick={() => {
            ...
            {quickReplies.map((qr, i) => (
              <div key={i} style={{ background: darkMode ? "#2C2C2E" : "#f9f9f9", borderRadius: 10, padding: "12px 14px", marginBottom: 10, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                ...
                <div style={{ display: "flex", gap: 8, marginLeft: 8 }}>
                  <button onClick={() => { setQrShortcut(qr.shortcut); setQrMessage(qr.message); setEditingIdx(i); }}
                    style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18 }}>✏️</button>
                  <button onClick={() => saveQR(quickReplies.filter((_, j) => j !== i))}
                    style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18 }}>🗑️</button>
                </div>
              </div>
            ))}
