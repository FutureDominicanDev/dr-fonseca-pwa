...
export async function POST(request: NextRequest) {
  try {
    ...
    const [{ data: inviteRes, error: inviteError }, { data: oneTimeInviteRes, error: oneTimeInviteError }] = await Promise.all([
      adminClient.from("app_settings").select("value").eq("key", "invite_code").maybeSingle(),
    ...
    const oneTimeInvite = findActiveStaffInviteCode(parseStaffInviteCodes(oneTimeInviteRes?.value), inviteCode);
