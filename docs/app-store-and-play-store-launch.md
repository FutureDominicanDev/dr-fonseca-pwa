...
- Native submission-prep update, May 16, 2026: iOS and Android now package `critical_repeat.wav` for native local notifications. Android also includes the notification status icon, Android 13 notification permission, camera/microphone/media permissions, and app/deep link intent filters. iOS includes camera, microphone, photo library, Face ID, background remote notification, and URL scheme metadata.
- Android build note: local debug APK and release AAB builds passed with a temporary Java 21 runtime and Android SDK 36. Permanent workstation setup should install JDK 21 and Android SDK platform/build-tools 36 before Play Console uploads.
- iOS build note: Swift Package dependencies resolve, but this Mac still needs the iOS 26.5 platform/simulator component installed in Xcode before simulator/device build verification can complete.
